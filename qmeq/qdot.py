...
from .config import verb_print, verb_print_
...
def construct_ham_coulomb(qd, coulomb, statelst, ham_=None):
    ...
    """
    verb_print_(3,f"qdot.py: construct_ham_coulomb() statelst: {statelst}")
...
def construct_ham_hopping(qd, hsingle, statelst, ham_=None):
    ...
    """
    verb_print_(3,f"qdot.py: construct_ham_hopping() statelst: {statelst}")
...
def construct_manybody_eigenstates(qd, hsingle, coulomb, statelst, ham_=None):
    ...
    """
    verb_print_(3,f"qdot.py: construct_manybody_eigenstates() statelst: {statelst}")
    if ham_ is not None:
        ham = ham_
    else:
...
class QuantumDot(object):
    ...
    def __init__(self, hsingle, coulomb, si,
        ...
        verb_print_(3,"qdot.py: QuantumDot.__init__()")
        self.si = si
        self.mtype = mtype
    ...
    def _init_hamiltonian(self):
        verb_print_(3,"qdot.py: QuantumDot._init_hamiltonian()")
        si = self.si
        if si.indexing == 'sz':
    ...
    def diagonalise_charge(self, charge=None, sz=None, ssq=None):
        ...
        ssq : int
            Total spin.
        """
        verb_print_(3,f"QuantumDot.diagonalise_charge() charge: {charge}  sz: {sz}  ssq: {ssq}")
        if charge is None:
            return None
        elif sz is None:
            chargelst = self.si.get_lst(charge)
